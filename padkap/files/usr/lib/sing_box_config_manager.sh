...
#     . /usr/lib/padkap/sing_box_config_manager.sh
#
...
sing_box_cm_patch_dns_route_rule() {
    ...
    echo "$config" | jq \
        ...
        --arg key "$key" \
        --argjson value "$value" \
        'import "helpers" as h {"search": "/usr/lib/padkap"};
        .dns.rules |= map(
...
#######################################
# Add a DoH blocking reject route rule with an inline ruleset containing
# known public DoH server IP ranges.
# Arguments:
#   config: string (JSON), sing-box configuration to modify
...
sing_box_cm_add_doh_block_route_rule() {
    ...
    local ruleset_tag="${tag}-ruleset"
    local cidrs_json
    cidrs_json=$(printf '%s' "$doh_ipv4_cidrs" | jq -R 'split(" ") | map(select(. != ""))')

    # Add inline ruleset with DoH server IPs
    ...
    # Add reject route rule referencing the ruleset
    config=$(echo "$config" | jq \
        --arg tag "$tag" \
        --arg inbound "$inbound" \
...
sing_box_cm_add_inline_ruleset_rule() {
    ...
    echo "$config" | jq -L /usr/lib/padkap \
        --arg tag "$tag" \
        --arg key "$key" \
        --argjson value "$value" \
        'import "helpers" as h {"search": "/usr/lib/padkap"};
