...
log_wget_failure() {
    ...
    if echo "$err" | grep -qi 'Operation not permitted'; then
        log "$operation got 'Operation not permitted'. On OpenWrt this can indicate firewall, routing, or IPv6 preference issues; padkap will retry with IPv4 when supported." "warn"
    fi
}
...
check_subscription_connectivity() {
    ...
    local attempt errfile rc family
    errfile="/tmp/padkap-subscription-check.$$"
    rm -f "$errfile"
    for attempt in $(seq 1 "$retries"); do
        family="any"
        if should_force_wget_ipv4 "$url"; then
