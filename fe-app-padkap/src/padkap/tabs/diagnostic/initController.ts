...
import { PADKAP_LUCI_APP_VERSION } from '../../../constants';
...
async function fetchSystemInfo() {
  const systemInfo = await PadkapShellMethods.getSystemInfo();
  ...
  if (systemInfo.success) {
    ...
    store.set({
      diagnosticsSystemInfo: {
        loading: false,
        padkap_version: _('unknown'),
        padkap_latest_version: _('unknown'),
...
async function handleRestart() {
  ...
  try {
    await PadkapShellMethods.restart();
  } catch (e) {
...
async function handleStart() {
  ...
  try {
    await PadkapShellMethods.start();
  } catch (e) {
...
async function handleEnable() {
  ...
  try {
    await PadkapShellMethods.enable();
  } catch (e) {
...
async function handleDisable() {
  ...
  try {
    await PadkapShellMethods.disable();
  } catch (e) {
...
async function handleShowGlobalCheck() {
  ...
  try {
    const globalCheck = await PadkapShellMethods.globalCheck();
...
async function handleViewLogs() {
  ...
  try {
    const viewLogs = await PadkapShellMethods.checkLogs();
...
async function handleShowSingBoxConfig() {
  ...
  try {
    const showSingBoxConfig = await PadkapShellMethods.showSingBoxConfig();
...
function renderDiagnosticAvailableActionsWidget() {
  ...
  const padkapEnabled = Boolean(servicesInfoWidget.data.padkap);
...
function renderDiagnosticSystemInfoWidget() {
  ...
  function getPadkapVersionRow(): IRenderSystemInfoRow {
    const loading = diagnosticsSystemInfo.loading;
    const unknown = diagnosticsSystemInfo.padkap_version === _('unknown');
    const hasActualVersion =
      Boolean(diagnosticsSystemInfo.padkap_latest_version) &&
      diagnosticsSystemInfo.padkap_latest_version !== 'unknown';
    const version = normalizeCompiledVersion(
      diagnosticsSystemInfo.padkap_version,
    );
    ...
    if (version !== `v${diagnosticsSystemInfo.padkap_latest_version}`) {
      logger.debug(
        '[DIAGNOSTIC]',
        'diagnosticsSystemInfo',
        diagnosticsSystemInfo,
  ...
  const renderedSystemInfo = renderSystemInfo({
    items: [
      ...
      {
        key: 'Luci App',
        value: normalizeCompiledVersion(PADKAP_LUCI_APP_VERSION),
      },
      {
        key: 'Sing-box',
