...
import { CustomPadkapMethods, PadkapShellMethods } from '../../methods';
...
async function fetchDashboardSections() {
  ...
  const { data, success } = await CustomPadkapMethods.getDashboardSections();
...
async function handleChooseOutbound(selector: string, tag: string) {
  await PadkapShellMethods.setClashApiGroupProxy(selector, tag);
...
async function handleTestGroupLatency(tag: string) {
  ...
  await PadkapShellMethods.getClashApiGroupLatency(tag);
  await fetchDashboardSections();
...
async function handleTestProxyLatency(tag: string) {
  ...
  await PadkapShellMethods.getClashApiProxyLatency(tag);
  await fetchDashboardSections();
...
async function renderServicesInfoWidget() {
  ...
  const renderedWidget = renderWidget({
    ...
    items: [
      {
        key: _('Padkap'),
        value: servicesInfoWidget.data.padkap
          ? _('✔ Enabled')
          : _('✘ Disabled'),
        attributes: {
          class: servicesInfoWidget.data.padkap
